				...
				<ul class="dropdown-menu fence-icon-wrap">
					...
					<li v-for="item in fenceTypeData" class="fence-li">
						<a @click.stop="showFence(item.ID)" href="javascript:;" class="clearfix">
							...
							{{item.name}}&nbsp;({{allFenceData==null?0:allFenceData[item.ID].length}})
							<i class="color-box"
				...
				<ul class="panel-body">
					...
					<li>
						...
						<el-switch :width="50"
								   active-text="_启用_"
								   inactive-text="_禁用_"
								   v-model="drawingData.state"></el-switch>
					</li>
			...
			initMap(){
				...
				switch (this.mapType) {
					case 0:
						...
						//实例化 测距工具
						this.myDis = new BMapLibDis.DistanceTool(this.map, {lineStroke: 5});
						//监听双击关闭事件
			...
			mapRemoveOverlay(overlay){
				...
				switch (this.mapType) {
					case 0:
						...
						overlay.removeEventListener("open",function () {
							_this.openInfowindowIndex=index;
						});
						overlay.removeEventListener("clickclose",function () {
...
// $("#locking").off("click");
				...
				if (points.length != 0)  {
					...
					//console.log("1",this.openInfowindowIndex);
			...
			showLately(data, index){
				...
				if (this.systemID == 2||this.systemID == 7) {
                    ...
                    // temp +=  
                    // <div class="BMap_bubble_header">
                    //     <i class="fa fa-lock" style="margin-right: 10px;color: #005fc6;font-size: 24px" v-show="false">
                    //     </i>${data.assetID}(${data.name})
                    // </div>
					// <div class="BMap_bubble_text">
					// <table>
					// 		<tbody>
					// 			<tr>
					// 				<td class="font-bolder text-right">_状态_：</td>
					// 				<td class="link-black">${status}</td>
					// 			</tr>
					// 			<tr>
					// 				<td class="font-bolder text-right">_车牌_：</td>
					// 				<td class="link-black">${data.name}</td>
					// 			</tr>;
					...
					temp += `<tr>
								<td class="font-bolder text-right">_里程_：</td>
								<td class="link-black">${data.mileage} km</td>
							</tr>
							<tr>
								<td class="font-bolder text-right">_速度_：</td>
								<td class="link-black">${data.speed} km/h</td>
							</tr>
							<tr>
								<td class="font-bolder text-right">_电量值_：</td>
								<td class="link-black">${data.batteryPercent}%</td>
							</tr>
								<td class="font-bolder text-right">_最后更新时间_：</td>
								<td class="link-black">${data.time}</td>
							</tr>
							<tr
							<tr>
					...
					switch (_this.systemID) {
						...
						case 4:
							...
							break;
						case 6:
							temp += `<tr><td class="font-bolder text-right">_箱状态_：</td><td class="link-black">${data.FIsLoaded}</td></tr>
							<tr>
								<td class="font-bolder text-right">_箱门状态_：</td><td class="link-black">${data.FIsOpenDoor}</td>
							</tr>
							<tr>
								<td class="font-bolder text-right">_内外温度_：</td><td class="link-black">${data.inTemperature} &nbsp;℃&nbsp;/&nbsp;${data.outTemperature}&nbsp;℃&nbsp;</td>
							</tr>`;
							break;
					}
					...
					if(_this.systemID==4){
						temp += `<tr><td class="font-bolder text-right">_温湿度_：</td><td class="link-black">`;
						for (var i = 0; i < data.coldChainEquipment.length; i++) {
							var obj = data.coldChainEquipment[i];
							temp += `<p style="display:inline-block;padding-right:10px">
					...
					<div class="BMap_bottom">
						<a carID="${data.ID}" carName="${data.name}"  class="InfoWindow_btn monitor cursor-pointer" title="_轨迹回放_"><i></i></a>
			...
			setMarks(items){
				...
				for (let i = 0; i < items.length; i++) {
					...
					let opts = {
						width : 250,     // 信息窗口宽度
						height: 90,     // 信息窗口高度
						title : "_电子锁_ ID："+obj.FAssetID , // 信息窗口标题
						...
						this.centerAndZoom(Mpoint, 18);
						marker.addEventListener("click",function(e){
							let content="_地址_："+obj.address;
							content+="<br/>_定位时间_:"+obj.FGPSDateTime;
							let  p = e.target;
							let  point = new BMap.Point(p.getPosition().lng, p.getPosition().lat);
							let Mpoint = _this.getMapPoint(point);
			...
			//还原路线
			showCarRoute(carPointData, points)  {
				...
				if(this.systemID==7){
					strokeStyle="dashed";
					startMarker = this.setMapMarker(startPoint, "/static/img/dispatching/origin.png", 4, 40, 38);
					endMarker = this.setMapMarker(endPoint, "/static/img/dispatching/car.png", 5, 42, 22);
				}else{
					strokeStyle="solid";
					...
					startMarker = this.setMapMarker(startPoint, "/static/img/carIcon/"+start_img, 4, 22, 29);
					endMarker = this.setMapMarker(endPoint, "/static/img/carIcon/"+end_img, 5, 22, 29);
				}
...
*/
			...
			mapShowRunPath(data){
				...
				console.log("���始结束点index",start,end);
				let highLightLine = this.allPointsArr.slice(start, end)
				//console.log(highLightLine);
				let option = {
					strokeColor: "#CC3333",
			...
			showCarAlarmPoint(StopData){
				...
				StopData.map(function (item, index) {
					let MPoint = _this.getMapPoint(item.point);
					let url = "/static/img/carIcon/alarm.png";
					let marker;
					let temp = item.alarmType+' '+item.keepTime;
					let color = "red";
					let label = _this.setMapLabel(temp, color);
					marker = _this.setMapMarker(MPoint, url, 15, 24, 24, label);
			...
			showPlayPoint(index){
				...
				if(data.point.lng==0 && data.point.lat==0){
			...
			showSite(data){
				...
				data.map(function (item,index) {
					...
					if(item.point.lng!==0&&item.point.lat!==0){
						...
						temp+=`</div>`;
						let textLabel = _this.setMapLabel(item.name, color)
					...
					$.ajax({
						...
						success: function (res) {
							if(res.FObject.Table1.length===0){
								_this.choicFenceType = '_请先创建围栏类型_';
								return
							};
							let obj = new Object();

							//临时根据语言类型编译围栏类型名称
							function parseFenceName(name){
								if(_this.$store.state.language=="CN") return name;
								switch(name){
								    case "工厂":
								        return "Factory";
									case "收费站":
							...
							res.FObject.Table1.map(function (item, index) {
								let data = {
									ID: item.FGUID,
									icon: item.FIcon,
									name: parseFenceName(item.FName),
						...
						$.ajax({
							...
							success: function (res) {
								...
								res.FObject.Table1.map(function (item, index) {
									...
									console.log(_this.mapType)
									let centerPoint=_this.parsePoint(_this.mapType,p);
									...
									if(item.FPath != "*"){
										...
										JSON.parse(item.FPath).map(function (item,index) {
											fencePath.push(_this.parsePoint(_this.mapType,item));
										})
									}
			...
			//父组件直接传入围栏类型和围栏数据
			transmitFenceData(fenceTypeData,fenceTypeObj,allFenceData){
				this.fenceTypeData=fenceTypeData;            //绘图类型 icon 颜色
				this.fenceTypeObj=fenceTypeObj;
				this.allFenceData=allFenceData;              //储存所有围栏信息
			},
			//服务器返回数据绘制围栏
			showFence(fenceType){
			...
			showOneFence(item, index, points){
				...
				//let closeIcon = this.setCloseBtn(item);   //绘制关闭图标
			...
			showfenceIcon(data, index){
				...
				let temp = `<div class="BMap_bubble_header text-blue font-20">${data.name}</div>
						<div class="BMap_bubble_text">
							<table>
								<tbody>
									<tr><td class="font-bolder text-right">_状态_：</td><td class="link-black ${data.state?"text-green":"text-red"}">${data.state?"_启用_":"_禁用_"}</td></tr>
			...
			showFencePlace(data){
				...
				let n = data.index;
				this.removeOverlay(this.fenceArr.graph, n);
				this.removeOverlay(this.fenceArr.icon, n);
				this.removeOverlay(this.fenceArr.closeIcon,n);
				this.showOneFence(data, n);
			},
			...
			setFence(type){
				...
				this.drawingData = {
					...
					formType: 0,     //类型  1圆 2矩形 3多边形
					fenceType: 0,
					path: null,     //多边形&&矩形 各顶���坐标数组  {lng:"",lat:""}
					radius: null,   //圆形半径
					center: {lng: null, lat: null},            //围栏近似中心点坐标
					address: null,                        //围栏近似中心点地址
				};
			...
			clickDeleteFence(obj){
				let ID = $(obj).attr("carId");
				let index=null;
				if($(obj).attr("carIndex")!=undefined){
		...
		computed: {
			...
			//计算滚动条百分百
			getPercent(){
				if(this.carPointData.length<=10){
					let n=Math.ceil(this.playNumber / (this.carPointData.length-1) * 100);
					if (n>100) {
						return 100;
					}else{
						return n;
					...
					return Math.ceil(this.playNumber / this.carPointData.length * 100);
				}
